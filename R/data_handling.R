...
#'
#' `collate_one_dat` parses one Vortex .dat or .stdat file, and returns the
...
#'
#' `collate_dat` collates all Vortex output files matching a given project
...
#' '.stdat', specify the scenario name. If `scenario=NULL`, all files with
#' extension .dat, matching the project name will be imported.
#'
#' `dir_in` may contain other files; only files matching the project (and,
...
#'
#' When `verbose=TRUE` the progress (i.e. the file being read) is reported
#' on screen.
#'
...
#'
#' `collate_run` collates all Vortex output files with extension .run
...
#'
#' `collate_yr` collates all the .yr output from Vortex matching the
...
#'
#' `collate_proc_data` collates multiple data frames generated by any of the
...
#' are filled with NA.
#'
#' When `save2disk=TRUE` the output file will be named 'CombinedDB'.
#' `dir_out` is created within the working directory unless a full path is
#' provided.
#'
#' @param data A list where each element is a df from `collate_[dat, yr, run]`
#' @inheritParams collate_dat
...
#' provided.
#'
#' @param data The df 'census' from `collate_yr`
...
#'
#' `lookup_table` creates a table that summarises simulation parameters.
...
#' population to be used as reference, otherwise `lookup_table` will look
#' for a population named 'Population 1' (i.e. Vortex default name for the first
#' population).
#'
#' `lookup_table` reports the values of SVs at year zero. This is done
...
#'
#' `SVs` can be any variable included in the data, including GS or PS set up
#' in Vortex.
#'
